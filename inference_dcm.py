...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    #header_font = ImageFont.truetype('./MSMINCHO.TTF',layout_engine=ImageFont.LAYOUT_BASIC)
    #main_font = ImageFont.truetype('./MSMINCHO.TTF',layout_engine=ImageFont.LAYOUT_BASIC)
    ...
    draw.text((10, 0), "HippoVolume.AI", (255, 255, 255))
    ...
    pil_i = Image.fromarray(nd_img, mode="L").convert("RGBA").resize(10,10)
...
def get_series_for_inference(path):
    ...
    for j in range(len(dicoms)):
        if (dicoms[j].SeriesDescription == "HippoCrop"):
            series_for_inference.append(dicoms[j])
...
#            series_for_inference.append(dicoms[i])
    ...
    #series_for_inference = [dcm_file for dcm_file in dicoms if dcm_file.SeriesDescription == 'HippoCrop']
...
if __name__ == "__main__":
    ...
    report_save_path = r"/home/wokspace/out/report.dcm"
